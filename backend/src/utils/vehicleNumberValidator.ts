// Валидация номеров автомобилей для разных стран
 ...
 * @param vehicleNumber - номер автомобиля для проверки
 * @returns результат валидации с указанием страны (если определена)
 */
export const validateVehicleNumber = (vehicleNumber: string): ValidationResult => {
  ...
  // (может быть нестандартный формат или другой формат)
...
/**
 * Форматирование номера для отображения (опционально)
 */
export const formatVehicleNumber = (vehicleNumber: string, country?: string): string => {
  if (!vehicleNumber) return '';
  
  const normalized = vehicleNumber.trim().replace(/\s+/g, '').toUpperCase();
  
  // Для РФ: А123БВ777 -> А 123 БВ 777
  if (country === 'RU' || /^[АВЕКМНОРСТУХ]\d{3}[АВЕКМНОРСТУХ]{2}\d{2,3}$/i.test(normalized)) {
    return normalized.replace(/^([АВЕКМНОРСТУХ])(\d{3})([АВЕКМНОРСТУХ]{2})(\d{2,3})$/i, '$1 $2 $3 $4');
  }
