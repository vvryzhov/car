...
Когда в GitHub появились новые изменения, обновите проект на сервере:
...
# Останавливаем контейнеры (опционально, можно не останавливать)
docker-compose down

# Получаем последние изменения из GitHub
git pull origin main

# Пересобираем образы с новыми изменениями
docker-compose build --no-cache

# Запускаем контейнеры
docker-compose up -d

# Или одной командой (останавливает, обновляет, пересобирает и запускает):
docker-compose down && git pull origin main && docker-compose build --no-cache && docker-compose up -d
```

**Важно:** 
- При обновлении база данных PostgreSQL сохраняется (данные в volume)
- Пользователи и заявки не теряются
- Если были изменения в структуре БД, они применятся автоматически при запуске
